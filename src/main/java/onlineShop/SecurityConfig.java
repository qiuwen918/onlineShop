...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			...
			.formLogin()
				.loginPage("/login")
				.failureUrl("/login?error=true")//fail to login，throw error
			...
			.and()
			.logout()
				.logoutUrl("/logout").logoutSuccessUrl("/login?logout=abc");
			
	}
	
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth
			.inMemoryAuthentication().withUser("qiuwtian@gmail.com").password("1234").authorities("ROLE_ADMIN");
